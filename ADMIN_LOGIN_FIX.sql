...
BEGIN
  ...
  SELECT id INTO admin_uid FROM auth.users WHERE email = 'admin@foodventurer.com';
  
  -- If user doesn't exist, create it
  IF admin_uid IS NULL THEN
    -- Note: You need to create the user via Dashboard first
    RAISE NOTICE 'User admin@foodventurer.com does not exist. Create it via Dashboard first.';
  ELSE
    -- Confirm email
    UPDATE auth.users
    ...
    VALUES (admin_uid, 'admin@foodventurer.com', 'Admin', 'User', 'super_admin', true)
...
WHERE u.email = 'admin@foodventurer.com';

-- Expected result:
-- email_confirmed: true
-- has_password: true
-- admin_email: admin@foodventurer.com
-- is_active: true
-- admin_level: super_admin
